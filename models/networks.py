...
def define_resnet_classifier(input_nc, num_classes, ngf, net_name, norm='batch', use_dropout=False, pool_type='max', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    Returns a resnet-based classifier
    """
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
...
class CnnClassifier(nn.Module):
    def __init__(self, input_nc, num_classes):
        ...
        self.classifier = nn.Sequential(
            # nn.Linear(32 * 56 * 56, 128) if num_classes == 4 else nn.Linear(32 * 8 * 8, 128), # 32 * 56 * 56 for (3*224*224) clip, 32 * 8 * 8 for (3*32*32) Icons-50. (Temporary solution)
